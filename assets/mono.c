...
int main (int argc, char** argv) {
  ...
  double drv_val[6];
  drv_val[0] = priors.decay_rate;
  drv_val[1] = priors.decay_rate;
  drv_val[2] = priors.decay_rate;
  drv_val[3] = priors.decay_rate == 0.0 ? 0.0 : 1.0 / priors.decay_rate;
  drv_val[4] = priors.decay_rate == 0.0 ? 0.0 : 1.0 / priors.decay_rate;
  drv_val[5] = priors.decay_rate == 0.0 ? 0.0 : 1.0 / priors.decay_rate;
  ...
  fwrite(signal, sizeof(double), count, out_file);
  fclose(out_file);
